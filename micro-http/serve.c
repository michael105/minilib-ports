...
#include <unistd.h>
#else
#define malloc(size) malloc_brk(size)
#endif
...
// Fix the location from which the files are served
char WEBROOT[256];
...
void print_request_sparse(const struct request_frame *request){
	...
	//fprintf(stderr, "XXX \n%s %s %s",  request->method, request->resource, request->protocol);
...
void response_generator(
	...
	struct stat path_stat;
	stat(full_resource_path, &path_stat);
	...
	//if (is_regular_file(full_resource_path)) {
		...
		for (int i = 0; i < sizeof(extensions)/sizeof(extn); ++i) {
			if (!strcmp(extensions[i].ext, this_extension)) {
				mime_set = 1;
				strcpy(res->headers[res->header_count].value, extensions[i].mimetype);
				break;
			}
		}
		if (! mime_set) {
			strcpy(res->headers[res->header_count].value, "text/plain");
			//strcpy(res->headers[res->header_count].value, "application/octet-stream");
		} 
	} else {
		// directory, or other trouble
